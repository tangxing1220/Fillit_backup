...
int	solve_a_square_tang(t_square *square, t_list **block, t_point *p_start)
{
	int found;
...
int	solve_squares_tang(t_list **tetri_lst)
{
	...
	if (!(board = create_square(board_size)))
		return (0);
	if (!(p_start = create_point(0)))
		return (0);
	while ((tetri_lst_tail = *tetri_lst) &&
			!solve_a_square(board, board_size, &tetri_lst_tail, p_start))
	{
		free_square(&square);
		if (!(square = create_square(++square_size)))
			return (0);
